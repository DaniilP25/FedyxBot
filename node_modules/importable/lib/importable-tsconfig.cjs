    ...
    return {
        create(info) {
            ...
            const createModuleResolver = (containingFile) => (moduleName, assertClause) => {
                const resolvedFileName = node_path.resolve(node_path.dirname(containingFile), moduleName);
                ...
                if (importablePlugins.testImportModuleSpecifier(resolvedFileName) &&
                    ...
                    const hashedFileName = `${node_path.sep}${moduleHash}${node_path.sep}..${resolvedFileName}`;
...
module.exports = importableTsconfig;
//# sourceMappingURL=importable-tsconfig.cjs.map
